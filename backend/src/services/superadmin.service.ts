...
/**
 * Crea una cuenta nueva con rol Administrador, vinculada a un condominio
 * existente (obligatorio: la tabla `usuario` exige un condominio_id_condominio,
 * y además así el Administrador queda con acceso inmediato a algo). Si
 * quiere administrar MÁS de un condominio, eso se hace después desde su
 * propia cuenta (crear/agregar condominios), no acá.
 */
export async function crearAdministrador(input: {
  ...
  const insert = await db
    ...
    .run(input.nombre_usuario, input.usuariocol, passwordHash, tipoAdminId.id_tipousuario, input.condominio_id_condominio);
  const id = Number(insert.lastInsertRowid);
  // Sin esto, el Administrador recién creado no tendría ninguna membresía
  // y no podría loguearse (ver auth.service.ts -> login).
